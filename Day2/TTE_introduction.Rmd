...
#    theme: cerulean
    ...
    font_adjustment: 3
    transition: none
    css: ../stylesheets/slidystyles.css
    duration: 45
...
aedat <-
  ...
  AE03 = ifelse(AETOXGR=="Severe", 1, 0)
  )
...
* The most common estimator of $H(t)$ is the `Nelson-Aalen` estimator
  - Can also estimate $S(t)$ as $\widehat{S(t)} = \exp\left\{ \widehat{H(t)} \right\}
...
2. For each interval $j$, 
    - calculate the proportion of subjects with an event ($d_j$), among the subjects in the <grn>risk set</grn> for that interval ($r_j$)
    - risk set at time $t$ = N - # events prior to $t$ - # censored prior to $t$
...
# Workbook: survfit estimation and plotting vs categorical and continuous predictors
