...
```{r}
dat.AE = dat.AE %>% filter(CAVGSS>0) %>% 
  mutate(Quartile = paste0("Q", ntile(CAVGSS, n = 4))) %>%
  select(ID,Quartile) %>% 
  right_join(dat.AE) %>% 
  mutate(Quartile=ifelse(CAVGSS==0,"Placebo",Quartile),
         Quartile=factor(Quartile,levels=c("Placebo",paste0("Q",1:4))))
...
summary(glm(AE ~ CAVGSS + ECOGF + cAGE, data=dat.AE, family='binomial'))
```
