...
## Bleeding events
First we'll plot time to first, second, and third bleeding events.  This is similar to Figure 1 in [@Abrantes2020-yd].
...
ggsurvplot_combine(list(First=fit1, Second=fit2, Third=fit3), risk.table = TRUE)
```
...
The divergent transition is a little troubling, but we can often fix a small numver of divergent transitions by increasing the `adapt_delta` parameter.  A larger number of divergent transitions indicates the geometry of the problem is problematic and you should reparameterize the model.
...
```{r}
eta_sample <- spread_draws(fit_base_noncentered_iiv,eta[ID], omega)
```

```{r}
d_post = eta_sample %>% 
  group_by(ID) %>% 
  summarise(est = median(eta*omega)) %>% 
  left_join(d_ind)
...
```{r}
pairs(fit_expresp, pars=c('log_alpha_pop','omega','log_gamma','beta'))
```
...
summary %>% 
  ...
  geom_violin() +
  labs(y='Medidn number of events over 90 days', x='Dose (IU/kg)')
```
