...
<!-- MW: Spell this out concretely how to get Y new. This formula makes it look far more complicated than it is -->
...
mod01_stan <- brm(formula = AE01 ~ CAVGSS + BWT + PTTYPE + SEXTXT,  
                  ...
                  chains = 4, 
                  inits = "random", 
                  cores = 4, # MW: Would it be easier to just set the mc.cores option?
                  seed = 123)
```
...
```{r}
# MW: Personal preference is that I don't like the default color scheme because its hard to tell chains apart.
# Could try bayesplot::color_scheme_set("viridis")
bayesplot::mcmc_trace(mod01_stan)
```

# Let's see the default priors in our model
<!-- MW: What's the point being made by this table? Its pretty confusing just to look at -->
```{r}
#prior_summary(mod01_stan)
```

# BRMS centers all predictors in the model
<!-- MW: Personal preference is to subscript the "_" parts -->
...
* Leave-one-out cross-validation (LOO-CV) to approximate ELPD
    ...
    * `loo` package
    <!-- MW: In the interest of time, I'd cut WAIC except for a passing mention. One of the big stan guys (gelman? Aki?) claims that LOO is always superior,primarily for its ability to tell when the estimates are not trustworthy -->
