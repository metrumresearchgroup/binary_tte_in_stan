...
```{r}
ggplot(dat_plus, aes(x = BWT, y = resd)) +
  geom_point() +
  geom_smooth() + 
  facet_wrap(~ STUDYID)
```
...
aedat_pp = bind_cols(aedat,
                     stats::simulate(mod03, nsim=500)) %>% # I changed nsim to 50 just to show that agreement between obs and pred isn't guaranteed
  pivot_longer(cols=sim_1:sim_500, values_to='value', names_to='name')

sim_summary <- aedat_pp %>% 
  group_by(name,SEXTXT) %>% 
  summarise(pAE = vpc_statistic(value)) %>% 
  mutate(type='Predicted')

comb_stat <- sim_summary %>% 
  bind_rows(obs_stat) %>% 
  group_by(type,SEXTXT) %>% 
  summarise(phat = median(pAE),
            lcl = quantile(pAE, probs = 0.05),
            ucl = quantile(pAE, probs = 0.95)) 

comb_stat %>% 
  ggplot(aes(x=SEXTXT, y=phat, group=type, col=type)) +
    geom_point() +
    geom_errorbar(aes(ymin=lcl, ymax=ucl)) +
    labs(y='Proportion with severe AE', x='Sex') 
```

```{r}
# The above fit seems to good to be true, but we can check that the average prediction within each group 
# actually does nearly equal the empirical rate within each group. (This is because of we happen to be fitting 
# using almost exactly the model that was used to generate the data.)
dat_mod$Pred <- predict(mod03, type = "response")

dat_mod %>% 
  group_by(SEXTXT) %>% 
  summarise(OBS = mean(AE01), 
            PRED = mean(Pred))
```
...
```{r}
obs_summary <- summary_function(aedat, .x_name = 'BWT', .y_name='AE01') %>% 
  mutate(type='Observed')
...
sim_summary <- aedat_pp %>% 
  ...
  slice(1:200) %>% 
  # Compute summary stats for each simulated dataset
  mutate(predictions = map(cols, ~summary_function(.x, .x_name='BWT'))) %>% 
  select(name, predictions) %>% 
  unnest(cols=predictions) %>% 
  # Summarise across simulated data sets
  group_by(xvar) %>% 
...
sim_summary %>% bind_rows(obs_summary) %>% 
  ...
  geom_line(aes(col=type, group=type)) +
  geom_ribbon(aes(ymin=qlo, ymax=qhi, fill=type), alpha=0.2) +
  labs(x='Body Weight (kg)', y='Probability of severe AE')

# no need to worry about the warning(s) -- they just indicate inability to fit a gam to some of the simulation reps
```
...
For brevity let's just focus on the VPCs with respect to CAVG : 

```{r}
# Simulate using the simulate function in stats
aedat_pp_04 = bind_cols(aedat,
                     stats::simulate(mod04, nsim=500)) %>% 
  pivot_longer(cols=sim_1:sim_500, values_to='value', names_to='name')

obs_summary <- summary_function(aedat, .x_name = 'CAVGSS', .y_name='AE01') %>% 
  mutate(type='Observed')

sim_summary <- aedat_pp_04 %>% 
  # Nest everying except the simulation name
  nest(cols=-name) %>% 
  # Use 200 sims for demonstration
  slice(1:200) %>% 
...
sim_summary %>% bind_rows(obs_summary) %>% 
  ...
  geom_line(aes(col=type, group=type)) +
  geom_ribbon(aes(ymin=qlo, ymax=qhi, fill=type), alpha=0.2) +
  labs(x='CAVGSS', y='Probability of severe AE')
```
```{r}
# Simulate using the simulate function in stats
aedat_pp_01 = bind_cols(aedat,
                     stats::simulate(mod01, nsim=500)) %>% 
  pivot_longer(cols=sim_1:sim_500, values_to='value', names_to='name')

obs_summary <- summary_function(aedat, .x_name = 'CAVGSS', .y_name='AE01') %>% 
  mutate(type='Observed')

sim_summary <- aedat_pp_01 %>% 
  # Nest everying except the simulation name
  nest(cols=-name) %>% 
  # Use 200 sims for demonstration
  slice(1:200) %>% 
...
The VPC for mod04 looks slightly better than the one for mod01. Let's see if AIC agrees: 

```{r}
AIC(mod01, mod04)
```

The above would suggest that the model with interaction has a better fit.

Interestingly, BIC does not agree in this case. As is expected generally, BIC puts a stiffer penalty on model complexity and so favors the model without interaction: 
