...
set.seed(1)
pp <- 0.7 # probability that evetn happens
...
Exercise: Apply linear transformations to `CAVGSS` to:
...
Generate values for `AE01` so that they conform to a probit-link, rather than a logit-link model.
...
dat_mod <- dat_use
mod01 <- glm(AE01 ~ CAVGSS + BWT + PTTYPE + SEXTXT,
...
TODO: RIG to add deviance residual plot for comparison with response resids

```{r, "TRY resid", purl=TRUE, results = "hide"}
dat_plus <- dat_mod
dat_plus$res <- residuals(mod02, type = "response")
dat_plus$pred <- fitted(mod02)
...
# deviance residuals
dat_plus$resd <- residuals(mod02, type = "deviance")
dat_plus$pred <- fitted(mod02)
...
TODO : Ramon to populate an example of DHARMa residual vs concentration
