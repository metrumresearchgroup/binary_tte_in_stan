...
$$
\begin{eqnarray*}
L(p \,\, | \,\, \mathbf{Y}=\mathbf{y}) & = & (\# \, \text{of "ones"}) \log(p)  \\ 
...
# Example plots: categorical with CI {.sourceCode.r.small}

```{r}
p <- aedat %>% group_by(STUDYID) %>% 
  summarise(x = sum(AE01), n=n()) %>% 
  mutate(phat = x / n,
         lcl = binom::binom.confint(x,n,methods = "wilson")$lower,
         ucl = binom::binom.confint(x,n,methods = "wilson")$upper) %>% 
  ggplot(aes(x=STUDYID, y=phat)) + 
  geom_col() +
