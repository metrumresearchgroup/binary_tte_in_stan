...
* For this series of classes we are going to use Stan to do Bayesian modeling
    ...
    * Stan is a probabalistic programming language for fitting Bayesian models.  
...
1. Construct a model for the data $p\left(y \,|\, \theta\right)$, conditional on parameters $\theta$

2. Assess prior distribution for $\theta$ -- $p\left(\theta\right)$
...
4. Sample from the posterior predictive distribution for $y$m  $p\left(y_\text{new} \,|\, y\right) = \int{p\left(y_{new}\,|\,\theta\right)p\left(\theta\,|\,y\right) d\theta}$.
...
- Total number of samples to take, including the warmup (`iter`)
