...
$$
    p(height \,|\, \mu, \sigma ) = \frac{1}{\sqrt{2\pi}
      \sigma} \exp\left\{-\frac{1}{2\sigma^2} \left(height - \mu \right)^2\right\}
$$
...
* It is the ***same expression*** as the PDF or pmf, but we now view it as a
...
::::{.column}
```{r, ref.label='cont-er', echo=FALSE, out.width="95%"}
...
```{r, echo=FALSE, message=FALSE, out.height="60%"}
data.frame(
  LP = seq(-6,6,.1)
) %>%
...
```{r, 'dhamrma-bin-resid',echo=FALSE, fig.show = 'hide'}
...
```{r, ref.label='dhamrma-bin-resid', echo=FALSE, out.width='95%'}
...
$$
r_{q,i} = \begin{cases}
  u \sim U(0,\hat{\pi}_i)  & \text{if } y_i = 0\\
  u \sim U(\hat{\pi}_i, 1) & \text{if } y_i = 1
\end{cases}
$$
:::
...
:::{.column}
```{r, ref.label='bayes-vpc', echo=FALSE, out.width="95%"}
