...
# odds of an AE for PT! versus HV are .21 lower
```
...
nsim = 100
sim_dat = posterior_linpred(mod03_stan) %>% 
  # applying expit function
  plogis() %>% 
  data.frame() %>% 
  slice(1:nsim)
sim_dat = apply(sim_dat,1,function(x) rbinom(length(x), 1, x))
