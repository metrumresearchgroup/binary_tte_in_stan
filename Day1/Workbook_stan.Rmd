...
* Look at convergence diagnostics:
    - Rhat and n_eff (`mcmc_plot(mod01_stan, type='rhat')`, `mcmc_plot(mod01_stan, type='neff')`)
...
* Look at the posterior summaries for parameters (`summary(mod01_stan)`).  Is there an effect of exposure?
    *  Think: How might you want to plot this effect?  We'll work on this later.

***Exercise:***

* Run a new model (mod02_stan) which adds `STUDYID`  to mod01_stan.
  That is, the new model will use `formula = AE01 ~ CAVGSS + PTTYPE + STUDYID` 
...
* Run a new model (mod03_stan) which modifies mod02_stan by adding normal prior distributions for the regression coefficients (i.e., class='b').
...
prop <- function(x) sum(x) / sum(!is.na(x))
...
nsim = 1000
sim_dat = add_predicted_draws(model=mod01_stan, 
                              newdata = dat_use)

head(sim_dat %>% as.data.frame())


sim_dat %>% 
  group_by(PTTYPE,Quartile,.draw) %>% 
  summarise(phat=mean(.prediction)) %>% 
  ggplot() + 
