...
model {
  ...
  for (i in 1:Nsubj){
    log_alpha[i] ~ normal(log_alpha_pop + log(1 - CAVGSS[i]/(CAVGSS[i]+EC50)), omega);
  }
...
generated quantities{
  ...
  for (i in 1:Nsubj){
    log_alpha_sim[i] = normal_rng(log_alpha_pop + log(1 - CAVGSS[i]/(CAVGSS[i]+EC50)), omega);
  }
}
