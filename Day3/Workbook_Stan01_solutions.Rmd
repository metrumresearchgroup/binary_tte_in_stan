...
```
> There are some divegent transitions, so we need to be cautious about interpreting the posterior summaries...
...
> Let's look at the pairs plot, highlighting the divergent transitions by putting them in the upper off-digonal plots.
...
mcmc_pairs(fit_lr_power, 
           ...
           np = nuts_params(fit_lr_power))                   # Where to find the divergent transition info
```
