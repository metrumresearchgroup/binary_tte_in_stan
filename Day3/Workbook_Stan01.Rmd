...
1. Copy the Stan model file to `../model/stan/logistic_regression_power.stan`

2. Modify the code to use a power model for SLD0 i.e., $\text{logit}\left(P(\text{ECOG}=1 | x)\right) = \alpha + \beta  x$.  You'll need to modify the parameters, model, and generated quantities blocks

3. did you get some post-warmup divergent transitions?  Follow the link provided by the Stan output (http://mc-stan.org/misc/warnings.html#divergent-transitions-after-warmup) to read more.
...
fit_lr_power <- stan(file = '../model/stan/logistic_regression_power.stan',
               data = stan_data,
               chains = 4,iter = 2000, warmup = 1000, 
               cores = 2, control = list(adapt_delta = 0.95))
```
